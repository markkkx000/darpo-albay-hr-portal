...
export function PositionCombobox({
    ...
    const handleSelect = (currentValue: string) => {
        const existing = uniquePositions.find(
    ...
    // Exact match check
    const hasExactMatch = uniquePositions.some(
    ...
    const displayName =
        value.id === 'new'
            ? value.name
            : positions.find((p) => p.id === value.id)?.name || value.name;
                ...
                >
                    ...
                    <CommandList>
                        <CommandEmpty>
                            {inputValue ? (
                                <div
                                    className="relative flex cursor-pointer items-center rounded-2xl px-2 py-1.5 text-sm transition outline-none select-none hover:item-hover-gradient"
                        ...
                        <CommandGroup>
                            {uniquePositions.map((position) => (
                                <CommandItem
                                    ...
                                    className="rounded-2xl transition aria-selected:item-hover-gradient"
                                >
                                    <Check
                                        className={cn(
                                            'mr-2 h-4 w-4',

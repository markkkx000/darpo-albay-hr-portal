...
export default function AuditIndex({ logs, filters, events, users }: Props) {
    ...
    const [startDate, setStartDate] = useState<string | null>(filters.start_date || null);
    const [endDate, setEndDate] = useState<string | null>(filters.end_date || null);
    ...
    const fetchLogs = (params: any = {}) => {
        const query: any = {
            ...
            start_date: params.startDate !== undefined ? params.startDate : startDate,
...
delete query.end_date;
        ...
        router.get(auditRoutes.index().url, query, { preserveState: true, replace: true, preserveScroll: true });
    };
    ...
    useEffect(() => {
        ...
        debounceRef.current = setTimeout(() => {
            if (search !== (filters.search || '')) {
                ...
                const query: any = { search, user_id: userId, event: event, start_date: startDate, end_date: endDate };
...
query.append('end_date', endDate);
    ...
    const getEventColor = (evt: string) => {
        switch (evt) {
            ...
            case 'auth': return 'bg-purple-500/10 text-purple-500 border-purple-500/20';
        ...
        <>
            ...
            <div className="p-4 w-full">
                <PageHeader
                    ...
                    actions={
                        <Button onClick={handleExport} className="btn-specular border-none">
                ...
                <Card className="matte-card elev-1 p-4 mb-4">
                    <div className="flex flex-col lg:flex-row gap-4 items-end">
                        ...
                        <div className="w-full lg:w-1/6">
                            <Select value={userId} onValueChange={(val) => handleFilterChange('userId', val)}>
                                <SelectTrigger>
                        ...
                        <div className="w-full lg:w-1/6">
                            <Select value={event} onValueChange={(val) => handleFilterChange('event', val)}>
                                <SelectTrigger>
                                    ...
                                    logs.data.map((log) => (
                                        <tr key={log.id} className="hover:bg-muted/40 transition-colors">
                                            <td className="px-6 py-4">
                                                ...
                                                <div className="text-[10px] tracking-tighter font-mono text-muted-foreground">{log.created_at_human}</div>
                                            </td>
                                            <td className="px-6 py-4 font-semibold text-foreground">{log.causer}</td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                {log.subject_type ? (
                                                    <div className="text-xs font-mono bg-surface px-2 py-1 rounded inline-block">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <Button
                                                    ...
                                                    className="btn-ghost-specular border-none h-8 px-3"
                                                >
            ...
            <Dialog open={!!selectedLog} onOpenChange={() => setSelectedLog(null)}>
                <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
                    ...
                    <div className="flex-1 overflow-auto bg-slate-950 p-4 rounded-md">
                        <pre className="text-emerald-400 font-mono text-xs whitespace-pre-wrap">
                            {selectedLog ? JSON.stringify(selectedLog.properties, null, 2) : ''}
                        </pre>
                    </div>
                </DialogContent>
            </Dialog>
...
AuditIndex.layout = {
    breadcrumbs: [
        { title: 'System Audit', href: auditRoutes.index().url },
    ],
};

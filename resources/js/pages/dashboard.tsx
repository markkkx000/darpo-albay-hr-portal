...
export default function Dashboard({ adminData, hrData, employeeData }: any) {
    ...
    const isHR =
        auth.permissions?.includes('personnel.view') ||
    ...
    return (
        <>
            <Head
                title={`${isSuperAdmin ? 'Admin' : isHR ? 'HR' : 'Employee'} Dashboard`}
            />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4 transition-opacity duration-300 ease-out md:p-6">
                <div className="mb-8 px-2">
                    <h1 className="mb-2 text-4xl font-extrabold tracking-tight text-foreground drop-shadow-sm">
                        {isSuperAdmin
                            ? 'Admin Dashboard'
                            : isHR
                              ? 'HR Dashboard'
                    ...
                    <p className="text-lg font-medium tracking-wide text-muted-foreground opacity-90">
                        {isSuperAdmin
                ...
                <div className="space-y-8">
                    ...
                    {!isSuperAdmin && (isSuperAdmin || isHR) && (
                        <HROverview data={hrData} employeeData={employeeData} />
                    )}
